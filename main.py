...
def evaluate_window(window, piece):
    ...
    # if we are checking from the player's perspective, then the opponent is AI
...
# displaying the initial menu to choose the game mode
def draw_menu():
    screen.fill(BLACK)
    ...
    screen.blit(title_label, (width // 2 - title_label.get_width() // 2, height // 4))

    single_player_label = menu_font.render("1. Single Player (vs AI)", 1, RED)
    screen.blit(single_player_label, (width // 2 - single_player_label.get_width() // 2, height // 2))

    two_player_label = menu_font.render("2. Two Players", 1, RED)
    screen.blit(two_player_label, (width // 2 - two_player_label.get_width() // 2, height // 2 + 50))

    pygame.display.update()

def update_move_display(move_time=None):
    global player_moves, ai_moves, game_mode
    total_moves = player_moves + ai_moves
    if game_mode == "AI":
        move_text = move_font.render(f"Player: {player_moves}  AI: {ai_moves}  Total: {total_moves}", 1, BLUE)
    else:
        move_text = move_font.render(f"Player 1: {player_moves}  Player 2: {ai_moves}  Total: {total_moves}", 1, BLUE)
    
    if move_time is not None:
        move_text = move_font.render(f"Player: {player_moves}  AI: {ai_moves}  Total: {total_moves}  Move Time: {move_time:.2f}s", 1, BLUE)
...
# additional font for move display
move_font = pygame.font.SysFont("monospace", 15)

# move counters
player_moves = 0
ai_moves = 0

# draw menu and wait for user input to start the game
draw_menu()
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION and not_over:
            ...
            elif turn == AI_TURN and game_mode == "Human":
    ...
    if turn == AI_TURN and not game_over and not_over and game_mode == "AI":
